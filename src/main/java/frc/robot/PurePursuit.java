...
public class PurePursuit {
	...
	public final PathConfig m_barrelConfig = new PathConfig(k_points, 8, 8, 15, k_maxJerk, k_dt, k_width);
	...
	private Field[] k_fields = { new Field("Vel", 'f'), new Field("Ideal Vel L", 'f'), new Field("Ideal Vel R", 'f'),
			...
			new Field("Curvature", 'f'), new Field("velDif", 'f'), new Field("cur Angle", 'f') };
	...
	public VelocityContainer followPath() {
		synchronized (lock) {
			...
			if (closestPathIdx > m_path.m_centerPath.length - k_checkStall && !m_isReversed) {// was 50
				if (Math.abs(getRightEncoderVel()) <= 0.1 && Math.abs(getLeftEncoderVel()) <= 0.1) {
					stopFollow();
			...
			if (distance < k_lookAheadDist) {
				...
				if (nextPosIdx < 0 && m_isExtended) {
					...
					double newX = newPos.x + k_extendedLookAhead
							* Math.cos(m_path.m_centerPath[m_path.m_centerPath.length - 1].heading);
					double newY = newPos.y + k_extendedLookAhead
							* Math.sin(m_path.m_centerPath[m_path.m_centerPath.length - 1].heading);
			...
			writer.write(velocity, (leftVel), (rightVel), -1 * getLeftEncoderVel(), getRightEncoderVel(), m_pos.x,
					m_pos.y, closestPos.x, closestPos.y, nextPos.x, nextPos.y, distance, dX, theta, curvature, velDif,
					angle);
			return new VelocityContainer(leftVel, rightVel);
		}
	}
	...
	private int getClosestPoint() {
		...
		int lookAheadPoints = m_prevIdx + k_lookAheadPoints > m_path.m_centerPath.length ? m_path.m_centerPath.length
				: m_prevIdx + k_lookAheadPoints;
	...
	private int getLookAheadPoint(double lookAheadFt) {
		...
		int lookAheadPoints = closestPoint + k_lookAheadPoints > m_path.m_centerPath.length ? m_path.m_centerPath.length
				: closestPoint + k_lookAheadPoints;
	...
	public class PathConfig {
		...
		public PathConfig(int points, double vel, double accel, double deccel, double jerk, double dt,
				double wheelBase) {
			m_points = points;
			m_vel = vel;
			m_accel = accel;
